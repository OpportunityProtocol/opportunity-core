...
//TODO: Define a relationship address
...
const workRelationshipABI = require('../artifacts/contracts/exchange/WorkRelationship.sol/FlatRateRelationship.json').abi
...
async function checkRelationshipStatus() {
    ...
    const accountBalance = await daiContractInstance.functions.balanceOf(accounts[0])
    ...
    let contractStatus, contractOwnership, contractState
    await workRelationshipContractInstance.functions.contractStatus().then(value => {
        contractStatus = value
    })

    await workRelationshipContractInstance.functions.contractState().then(value => {
        contractState = value
    })

    await workRelationshipContractInstance.functions.contractOwnership().then(value => {
        contractOwnership = value
    })
